...
import (
	...
	"github.com/TylerAldrich814/MetaReviews/pkg/discovery/memory"
	"github.com/google/go-cmp/cmp"
	"github.com/google/go-cmp/cmp/cmpopts"
	"google.golang.org/grpc"
	"google.golang.org/grpc/credentials/insecure"

	metadatatest "github.com/TylerAldrich814/MetaReviews/metadata/pkg/testutil"
	movietest "github.com/TylerAldrich814/MetaReviews/movie/pkg/testutil"
	ratingtest "github.com/TylerAldrich814/MetaReviews/rating/pkg/testutil"
)
...
func main(){
  ...
  metadataServer := startMetadataService(ctx, registry)
  defer metadataServer.GracefulStop()
  ratingServer := startRatingService(ctx, registry)
  defer ratingServer.GracefulStop()
  movieServer := startMovieService(ctx, registry)
  defer movieServer.GracefulStop()
  ...
  m := &gen.Metadata {
    ...
    Description : "A movie you've never watched.",
    Director    : "TAA",
  }
  if _, err := metadataClient.PutMetadata(
    ctx, 
  ...
  getMetadataResp, err := metadataClient.GetMetadata(
    ctx, 
  ...
  if err != nil {
    log.Fatalf("get metadata: %v\n", err)
  }
  if diff := cmp.Diff(
    getMetadataResp.Metadata, 
    m, 
    cmpopts.IgnoreUnexported(gen.Metadata{}),
  ); diff != "" {
    log.Fatalf("get metadata after put mismatch: %v\n", err)
  }

  // -----------------------------
  // -----------------------------
  // -----------------------------
  log.Printf("Getting Movie Details via movie service")
  wantMovieDetails := &gen.MovieDetails{
    Metadata: m,
  }
  getMovieDetailsResp, err := movieClient.GetMovieDetails(
    ctx,
    &gen.GetMovieDetailsRequest{
      MovieId: m.Id,
    },
  )
  if err != nil {
    log.Fatalf("get movie details: %v\n", err)
  }

  if diff := cmp.Diff(
    getMovieDetailsResp.MovieDetails, 
    wantMovieDetails,
    ...
    log.Fatal("get movie details after put mismatch: %v\n", err)
  }
  ...
  firstRating := int32(5)
  if _, err := ratingClient.PutRating(
    ctx,
  ...
  log.Println("Retreiving initial aggregated rating via rating service...")

  getAggregatedRatingResp, err := ratingClient.GetAggregatedRating(
    ctx,
  ...
  if err != nil {
    log.Fatal("get aggregated rating: %v\n", err)
  }

  if got, want := getAggregatedRatingResp.RatingValue, float64(5); got != want {
    log.Fatalf("rating mismatch: Got %v want %v\n", got, want)
  }
  ...
  if err != nil {
    log.Fatalf("get aggregated rating: %v\n", err)
  }
  ...
  if diff := cmp.Diff(
    ...
    log.Fatalf("get movie details after update mismatch: %v\n", err)
  }
...
func startMetadataService(
  ...
  log.Printf("Starting metadata service on %s\n", metadataServiceAddr)
  h := metadatatest.NewTestMetadataGRCServer()
...
func startRatingService(
  ctx      context.Context,
  registry discovery.Registry,
) *grpc.Server {
  log.Printf("Starting rating service on %s\n", ratingServiceAddr)
...
func startMovieService(
  ...
  log.Printf("Starting movie service on %s\n", movieServiceName)
